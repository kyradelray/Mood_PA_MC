...
# Create active variable: 1 if steps > 7000 OR workout_duration_minutes >= 30
cycle_days_with_activity['active'] = (
    (cycle_days_with_activity['steps'] > 7000) |
    (cycle_days_with_activity['workout_duration_minutes'] >= 30)
).astype(int)

# Set active to NaN where both steps and workout are missing
both_missing = (
    cycle_days_with_activity['steps'].isna() &
    cycle_days_with_activity['workout_duration_minutes'].isna()
)
cycle_days_with_activity.loc[both_missing, 'active'] = np.nan

print(f"\nMatched records with activity data: {cycle_days_with_activity['steps'].notna().sum()}")
print(f"Active variable distribution:")
